...
namespace caelumdemo
{
    public class Camera
    {
        ...
        private List<caelum4csharp.ICameraBound> attachedObjects = new List<caelum4csharp.ICameraBound>();
        ...
        /// <summary>Attach an object that will move with this camera position.</summary>
        /// <param name="obj">The object to bind to this camera position.</param>
        public void AttachObject(caelum4csharp.ICameraBound obj)
        {
            if (attachedObjects.Contains(obj)) return;
            else attachedObjects.Add(obj);

            obj.UpdatePosition(position);
        }

        /// <summary>Detch an object from this camera.</summary>
        /// <param name="obj">The object to detach from this camera.</param>
        public void DetachObject(caelum4csharp.ICameraBound obj)
        {
            if (attachedObjects.Contains(obj)) attachedObjects.Remove(obj);
        }

        /// <summary>Update any object that is attached ot this camera.</summary>
        private void UpdateAttachedObjects()
        {
            foreach (var obj in attachedObjects) obj.UpdatePosition(position);
        }
    }
}
